...
```bash
python <skill-dir>/arxiv_download.py <arxiv_id>
```
...
```bash
python <skill-dir>/arxiv_download.py 2501.00663
# → <skill-dir>/cache/2501.00663.html
```
...
- **キャッシュ**: 2回目以降は即座（~0.1秒）で返る。キャッシュ先は既定でスキルディレクトリ内の `cache/`。環境変数 `ARXIV_CACHE_DIR` を設定すると別の場所を使う（既存キャッシュの再利用などに）。
